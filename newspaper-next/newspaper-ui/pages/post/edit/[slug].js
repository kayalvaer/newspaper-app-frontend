...
const EditPost = ({ post }) => {
  ...
  const [values, setValues] = useState({
    ...
    img: post.attributes.img.data.attributes.id,
  });
  const [imageURL, setImageURL] = useState(
    process.env.NEXT_PUBLIC_STRAPI_URL + post.attributes.img.data.attributes.url
  );
  ...
  const handleImageUpload = async (e) => {
    ...
    setImageURL(process.env.NEXT_PUBLIC_STRAPI_URL + responseData[0].url);
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/posts/${post.id}`,
      {
        method: "PUT",
        headers: {
